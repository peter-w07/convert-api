...
const server = app.listen(PORT, HOST, () => {
  ...
  log.info(`  routes: /api/screenshot /api/convert /api/convert/batch /api/ytdlp /api/ocr /api/transcribe /api/jobs /api/formats /discord`);
...
async function closeHttpServer(timeoutMs = 10_000): Promise<void> {
  await new Promise<void>((resolve) => {
    ...
    const timer = setTimeout(() => {
      log.warn(`HTTP shutdown exceeded ${timeoutMs}ms; closing active connections`);
