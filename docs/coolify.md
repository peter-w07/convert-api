...
2. **Source**: GitHub → `peter-w07/convert-api` → branch `main` (or `claude/inspiring-cannon-ChwPJ` for the development branch).
...
# Render a screenshot
curl -X POST "$DOMAIN/api/screenshot" \
...
In Coolify, **Resource → Webhooks** gives you a URL you can paste into the GitHub repository's webhook settings. Pushes to `main` will auto-redeploy.
...
Push a new commit → Coolify rebuilds → restart. To pull and rebuild manually, hit **Redeploy** in the Coolify UI.
